...
export function RobotAnimation() {
  ...
  useEffect(() => {
    ...
    }, 100); // Robot move speed
    return () => clearInterval(interval);
  }, []);

  return (
    <div className="relative w-full h-64 flex items-center overflow-hidden bg-gray-900">
      {/* Robot */}
      <motion.div
        className="absolute flex items-center"
        style={{ left: `${position}%` }}
      ...
      {hasLaunchText && (
        <motion.div
          className="absolute left-1/2 top-12 -translate-x-1/2 flex flex-col items-center space-y-2 z-10"
          initial={{ scale: 0, opacity: 0 }}
          animate={{ scale: [0, 1.3, 1], opacity: 1 }}
          transition={{ duration: 0.8 }}
        >
          <div className="relative">
            <div className="bg-black/90 rounded-lg px-6 py-3 border border-blue-400/60 text-3xl font-extrabold text-white drop-shadow-lg animate-bounce">
              🚀 Launched Now!
            </div>
            {/* Sparkle effects */}
            <motion.div
